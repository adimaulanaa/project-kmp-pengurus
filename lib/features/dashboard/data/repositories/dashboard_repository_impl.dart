...
class DashboardRepositoryImpl implements DashboardRepository {
  ...
  @override
  Future<Either<Failure, DashboardChartModel>> dashboardChart(
      CashBookData dataChart) async {
    if (await networkInfo.isConnected) {
      try {
        final remoteData = await remoteDataSource.getDashboardChart(dataChart);
        await localDataSource.cacheDashboardChart(remoteData);
        return Right(remoteData);
      } on BadRequestException catch (e) {
        return Left(BadRequestFailure(e.toString()));
      } on UnauthorisedException catch (e) {
      ...
      try {
        final localTrivia = await localDataSource.getLastCacheDashboardChart();
        return Right(localTrivia);
      } on CacheException {
        return Left(CacheFailure());
      }
  ...
  @override
  Future<Either<Failure, DashboardChartModel>> dashboardChartFromCache() async {
    try {
      final localTrivia = await localDataSource.getLastCacheDashboardChart();
      return Right(localTrivia);
    } on CacheException {
  ...
  Future<Either<Failure, GuestBookModel>> guestBook() async {
    if (await networkInfo.isConnected) {
      ...
      try {
        final localTrivia = await localDataSource.getLastCacheGuestBook();
        return Right(localTrivia);
      } on CacheException {
        return Left(CacheFailure());
      }
  ...
  @override
  Future<Either<Failure, VisitorModel>> allGuestBook() async {
    if (await networkInfo.isConnected) {
      try {
        final remoteData = await remoteDataSource.getAllGuestBook();
        await localDataSource.cacheAllGuestBook(remoteData);
        return Right(remoteData);
      } on BadRequestException catch (e) {
        return Left(BadRequestFailure(e.toString()));
      } on UnauthorisedException catch (e) {
      ...
      try {
        final localTrivia = await localDataSource.getLastCacheAllGuestBook();
        return Right(localTrivia);
      } on CacheException {
        return Left(CacheFailure());
      }
