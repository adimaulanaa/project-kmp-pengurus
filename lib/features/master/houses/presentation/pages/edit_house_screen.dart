...
class EditHouse extends StatefulWidget {
  final Houses? data;
  const EditHouse({Key? key, required this.data}) : super(key: key);
...
class _EditHouseState extends State<EditHouse> {
  ...
  @override
  void initState() {
    statusRumah = widget.data!.isVacant!;
    gender = widget.data!.citizenGender!.toString();
    if (gender == 'L') {
      isMale = true;
      isFemale = false;
    } else if (gender == 'P') {
    ...
    widget.data!.subscriptions!.forEach((element) {
      jenisIuran.add(element.subscription!.id!);
    });

    rtrw = widget.data!.rtPlace!.id!;
    nmJalanCtr.text = widget.data!.street!;
    nmBlok = widget.data!.houseBlock!;
    noRumahCtr.text = widget.data!.houseNumber!;
    nmBlokCtr.text = widget.data!.houseBlock!;
    noKtpCtr.text = widget.data!.citizenIdCard!;
    nmPenghuniCtr.text = widget.data!.citizenName!;
    noTelpCtr.text = widget.data!.citizenPhone!;
    statusPenghuni = widget.data!.isActive!;
    gratisIuran = widget.data!.isFree!;

    if (statusPenghuni == true) {
      pemilik = true;
      kontrak = false;
    } else if (statusPenghuni == false) {
      kontrak = true;
      pemilik = false;
    }

    super.initState();
    BlocProvider.of<HousesBloc>(context).add(EditLoadHouses());
  }
    ...
    return BlocListener<HousesBloc, HousesState>(
      listener: (context, state) async {
        if (state is HousesLoading) {
          ...
          setState(() {});
        } else if (state is EditHousesLoaded) {
          final progress = ProgressHUD.of(context);
          if (state.data!.isNotEmpty && state.data!.length > 0) {
            ...
            for (var item in jenisIuran) {
              for (var itemIuran in listIuran) {
                if (item == itemIuran.id) {
                  itemIuran.check = true;
                }
              }
            }
          ...
          _thankYouPopup();
        }else if (state is DeleteHousesSuccess) {
          _deletePopup();
        }
                  ...
                  InkWell(
                    onTap: () {
                      ...
                      BlocProvider.of<HousesBloc>(context)
                          .add(DeleteHousesEvent(idHouse: widget.data!.id!));
                      progress.dismiss();
                    },
                    child: Container(
                        child: Text("Hapus",
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          FormBuilderDropdown(
                            initialValue: widget.data!.village!.id,
                            name: 'kampung',
                            onChanged: (val) {
                          ...
                          FormBuilderDropdown(
                            initialValue: widget.data!.rtPlace!.id!,
                            name: 'rt',
                            onChanged: (value) {
                              rtrw = value.toString();
                            },
        ...
        onPressed: pagePosition != 2.0
            ...
            : () {
                if (_formKey.currentState!.validate()) {
                  ...
                  BlocProvider.of<HousesBloc>(context)
                      .add(EditHousesEvent(housesEdit: itemHouses));
                  progress.dismiss();
                } else {
                  final progress = ProgressHUD.of(context);
                  progress!.showWithText(GlobalConfiguration()
