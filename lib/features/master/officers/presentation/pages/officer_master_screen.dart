...
import 'package:kmp_pengurus_app/features/master/officers/presentation/pages/add_officer.dart';
import 'package:kmp_pengurus_app/features/master/officers/presentation/pages/edit_officer.dart';
...
class PetugasScreen extends StatefulWidget {
  ...
  PetugasScreen({Key? key, required this.isPic, required this.isTreasurer})
      : super(key: key);
...
class _PetugasScreenState extends State<PetugasScreen> {
  ...
  List<Officers> dummySearchList = [];
  List<Officers> searchList = [];
  ...
  void initState() {
    ...
    BlocProvider.of<OfficersBloc>(context).add(LoadOfficers());
  }
    ...
    return BlocListener<OfficersBloc, OfficersState>(
      listener: (context, state) async {
        if (state is OfficersLoading) {
          isLoading = false;
          setState(() {});
        } else if (state is OfficersLoaded) {
              ...
              return Column(
                children: [
                  Container(
                      ...
                      child: customListPetugas(context, index)),
                ],
              );
            ...
            ? () async {
                await Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) => BlocProvider(
                              create: (context) =>
                                  serviceLocator.get<OfficersBloc>(),
                              child: ProgressHUD(child: AddPetugasScreen()),
                            )));
                BlocProvider.of<OfficersBloc>(context).add(LoadOfficers());
              }
            : () {
            ...
            delegate:
              ...
              return Column(
                children: [
                  Container(
                      padding: EdgeInsets.only(left: 20, right: 20),
                      child: customListPetugasSearch(context, index)),
                ],
              );
  ...
  Widget customListPetugas(BuildContext context, int index) {
    if(isPic!){
      if(isTreasurer!){
  ...
  Widget customListPetugasSearch(BuildContext context, int index) {
    return InkWell(
      onTap: isPic!
          ? () async {
              await Navigator.push(
